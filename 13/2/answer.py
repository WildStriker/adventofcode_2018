...
class Cart:
    ...
    def on_track(self) -> bool:
        return self._tracks[self._coords].cart is self
...
def get_last(carts: typing.List[Cart]):
    while len(carts) > 1:
        carts.sort(key=compare_cart)
        new_carts = carts[:]
        for cart in carts:
            if cart.on_track():
                other_cart = cart.move_cart()
                if other_cart:
                    new_carts.remove(other_cart)
